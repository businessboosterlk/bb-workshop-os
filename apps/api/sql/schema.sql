...
create table if not exists wos_records (id uuid primary key default gen_random_uuid(), client text not null references wos_clients(slug), kind text not null check (kind in ('jobs','customers','activities','photos','enquiries','quotes')), data jsonb not null default '{}', created_at timestamptz default now(), updated_at timestamptz default now());
